...
<head>
    ...
    <style>
        ...
        .container { background: var(--card); border: 1px solid var(--border); padding: 30px; border-radius: 8px; width: 100%; max-width: 450px; box-shadow: 0 0 20px rgba(0,255,65,0.1); }
        h1 { color: var(--accent); font-size: 18px; text-align: center; margin-bottom: 20px; }
        .user-info { text-align: center; margin-bottom: 20px; padding: 10px; border-bottom: 1px dashed var(--border); color: #888; font-size: 14px; }
        ...
        .stat-value { font-size: 20px; color: var(--accent); font-weight: bold; }
...
app.get('/', (req, res) => {
    if (!fs.existsSync('.env')) {
        ...
        res.send(htmlTemplate("Dashboard", `
            <h1>> SNIPER_ONLINE</h1>
            <div class="user-info">ACCOUNT: <span style="color:var(--accent)">${user}</span></div>
            <div class="stats">
                ...
                <div class="stat-box" style="grid-column: span 2;">
                    ...
                    <div class="stat-value" style="font-size: 30px;">฿ ${stats.amount.toFixed(2)}</div>
                </div>
            </div>
            <button onclick="location.href='/reset'" class="btn-reset">RESET SYSTEM</button>
...
app.listen(10000, () => {
    console.log("Web UI: http://localhost:10000");
